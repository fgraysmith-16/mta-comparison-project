...
All credit allocated to the last channel in the customer journey.
"""
...
def apply_last_touch(
    ...
    """
    Apply last-touch attribution to journeys.
    ...
    Returns:
        pd.DataFrame with columns:
            - journey_id
            - channel
            - attributed_revenue: Revenue allocated to this channel (0 for non-last)
            - attributed_conversions: 1 if this channel is last-touch, else 0
    """
    rows = []
    ...
    for _, journey in journeys_df.iterrows():
        ...
        # Last touch channel
        last_channel = touches[-1][1]
        
        # All channels get 0, except last which gets 100%
        for _, channel in touches:
            rows.append({
                "journey_id": journey_id,
                "channel": channel,
                "attributed_revenue": revenue if channel == last_channel else 0.0,
                "attributed_conversions": 1.0 if channel == last_channel else 0.0,
            })
...
if __name__ == "__main__":
    ...
    print("Applying last-touch attribution...")
    result = apply_last_touch(journeys)
